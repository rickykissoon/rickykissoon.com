...
import { commitSession, getKeyValue, setKeyValue } from "~/sessions.server";
...
export default function ServerStateUseFetcher() {

    return(
        <div>
            <div>
                <h1 className="text-[25px] font-bold mb-5 text-center">Server As State: useFetcher()</h1>
            </div>
            <p>
                useFetcher() is a hook for interacting with loaders and actions without navigating, changing the URL or affecting browser 
                history. It can be used as a component (like {'<Form>'}), or it can be used directly through one of its built in methods.
            </p>
            <br></br>
            <p>
                The biggest benefits of useFetcher() is that it allows you to read or write data without causing URL
                changes or navigations. It's perfect for small interactions that shouldn&apos;t be bookmarkable or shareable, 
                that dont need full-page revalidations, or when you need to make multiple isolated requests on the same page.
            </p>
            <br></br>
            <p>
                We'll illustrate these benefits in the following example by using both the component and method forms of
                ...
                <div>
                    <svg
                        onClick={() => fetcher.submit(
                            { intent: "toggleStar", movieId: String(initial.id) },
                            { method: "post" }
                        )}
                        xmlns="http://www.w3.org/2000/svg"
                        fill={starred ? "yellow" : undefined }
                        viewBox="0 0 24 24"
                        strokeWidth={1.5}
                        stroke="currentColor"
                        className="stroke-yellow-400 size-6 cursor-pointer"
                    >
...
export function getPrefsFromKeyValue(keyValue: Record<string, unknown>): Prefs {
